...
export async function createSalesProposalData(formData: FormData) {
  const store = withSales(await readStore());
  ...
  return { hubId, proposalId: proposal.id, publicLink: proposal.publicLink };
}

export async function updateSalesProposalStatusData(formData: FormData) {
  const store = withSales(await readStore());
  ...
  await saveStore(store);
  return { hubId, proposalId };
}

export async function updateSalesProposalPricingData(formData: FormData) {
  const store = withSales(await readStore());
  const proposalId = text(formData.get("proposalId"));
...
export async function addSalesProposalResponseData(formData: FormData) {
  const store = withSales(await readStore());
  const proposalId = text(formData.get("proposalId"));
  ...
  await saveStore(store);
  return { hubId: proposal.hubId, proposalId, publicLink: proposal.publicLink };
}


export async function createGroupedSalesProposalData(formData: FormData) {
  const store = withSales(await readStore());
  ...
  const results: Array<{ hubId: string; proposalId: string; publicLink: string }> = [];
  for (const hubId of selectedHubIds) {
    ...
    results.push(await createSalesProposalData(next));
  }
  return { groupId, proposals: results };
}
