...
const EMPTY_STORE: Store = { hubs: [], clientes: [], solicitudes: [], hub_servicios: [], hub_servicio_vinculos: [], reportes: [] };
...
function backupFileName(suffix = timestampForFile()) {
  const parsed = path.parse(DATA_FILE);
  return path.join(parsed.dir, `${parsed.name}.backup-${suffix}${parsed.ext}`);
}

async function backupStoreFile(reason = "manual") {
  try {
    await mkdir(path.dirname(DATA_FILE), { recursive: true });
    const backupPath = backupFileName(`${timestampForFile()}-${reason}`);
    await copyFile(DATA_FILE, backupPath);
    return backupPath;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      console.error("No se pudo crear backup del store", { dataFile: DATA_FILE, error: (error as Error).message });
    }
    return null;
  }
}
...
function parseJsonPrefix(text: string) {
  ...
  const startChar = text[trimmedStart];
  ...
  for (let index = trimmedStart; index < text.length; index += 1) {
    ...
    else if (char === startChar) depth += 1;
    else if (char === closeChar) {
      depth -= 1;
      if (depth === 0) {
        const candidate = text.slice(trimmedStart, index + 1);
        try {
          return JSON.parse(candidate) as Partial<Store>;
        } catch {
          return null;
        }
      }
...
async function findLatestValidBackup(): Promise<Partial<Store> | null> {
  try {
    const dir = path.dirname(DATA_FILE);
    const parsed = path.parse(DATA_FILE);
    const backups = (await readdir(dir))
      .filter((file) => file.startsWith(`${parsed.name}.backup-`) && file.endsWith(parsed.ext))
      .sort()
      .reverse();

    for (const backup of backups) {
      try {
        return JSON.parse(await readFile(path.join(dir, backup), "utf8"));
      } catch {
        // Ignorar backups inválidos y continuar con el anterior.
      }
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      console.error("No se pudieron revisar backups del store", { dataFile: DATA_FILE, error: (error as Error).message });
    }
  }
  return null;
}
...
async function readStore(): Promise<Store> {
  try {
    ...
    console.error("Store JSON corrupto", { dataFile: DATA_FILE, error: (error as Error).message });
    const corruptBackup = await backupStoreFile("corrupto");
    if (corruptBackup) console.error("Backup del store corrupto creado", { dataFile: DATA_FILE, backupFile: corruptBackup });

    try {
      const repaired = parseJsonPrefix(await readFile(DATA_FILE, "utf8"));
      if (repaired) {
        const store = normalizeStore(repaired);
        await saveStore(store, { backupBeforeWrite: false });
        console.error("Store JSON reparado desde el primer documento JSON válido", { dataFile: DATA_FILE });
        return store;
      }
    } catch (repairError) {
      console.error("No se pudo reparar automáticamente el store corrupto", { dataFile: DATA_FILE, error: (repairError as Error).message });
    }

    const backup = await findLatestValidBackup();
    if (backup) {
      console.error("Store JSON cargado desde el último backup válido", { dataFile: DATA_FILE });
      return normalizeStore(backup);
    }

    console.error("Store JSON corrupto sin backup válido; se usará estructura vacía segura sin borrar el archivo corrupto", { dataFile: DATA_FILE });
    return normalizeStore(EMPTY_STORE);
  }
}

async function saveStore(store: Store, options: { backupBeforeWrite?: boolean } = {}) {
  await mkdir(path.dirname(DATA_FILE), { recursive: true });
  if (options.backupBeforeWrite !== false) await backupStoreFile("pre-write");
