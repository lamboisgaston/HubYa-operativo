...
export type Hub = { id: string; nombre: string; slug: string; zona: string; estado: HubEstado; categoriaId: HubCategorySlug; tipo: HubCategorySlug; descripcionPublica: string; rama: string; equipoOperativo: string; moduloOperativo?: ModuloOperativoHub; parametrosOperativos?: ParametrosOperativosHub; hubOperativo?: IntegranteHubOperativo[]; nivelEstabilidad?: number; activo: boolean; informacionImportante?: HubInformacionImportante; trabajosRealizados?: number; ultimaActividad?: string; createdAt: string; updatedAt: string };
...
export type NuevoHubDemandaInput = { nombre: string; zona: string; categoriaId?: HubCategorySlug; descripcionPublica?: string; rama?: string; equipoOperativo?: string };
...
const seedHubs: Hub[] = ["Tipal", "Punto", "Praderas", "Valle Escondido", "Chacras de Santa María", "La Aguada", "Prado", "La Reserva"].map((zona, index) => ({ id: `hub-${index + 1}`, nombre: `Hub ${zona}`, slug: zona.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""), zona, estado: "activo", categoriaId: HUB_CATEGORY_DEFAULT, tipo: HUB_CATEGORY_DEFAULT, descripcionPublica: `Hub operativo de ${zona} para coordinar demanda recurrente, equipo disponible y trabajos programados sin exponer datos privados de clientes.`, rama: "JardinerosYa", equipoOperativo: "JardinerosYa01", moduloOperativo: "jardinerosya", parametrosOperativos: { jardinerosYa: parametrosJardinerosYaVacios() }, hubOperativo: [], nivelEstabilidad: 8, activo: true, trabajosRealizados: 0, ultimaActividad: now, createdAt: now, updatedAt: now }));
...
export async function createHubDemanda(input: NuevoHubDemandaInput) { const store = await readStore(); const categoria = getHubCategoryConfig(input.categoriaId); const timestamp = new Date().toISOString(); const nombre = nombreHubCanonico(input.nombre.trim() || input.zona.trim() || "nuevo"); const zona = input.zona.trim() || nombre.replace(/^Hub\s+/i, ""); const claveNueva = normalizarClaveHub(nombre); const existente = store.hubs.find((hub) => normalizarClaveHub(hub.nombre) === claveNueva || normalizarClaveHub(hub.zona) === claveNueva); if (existente) return { ...existente, clientesActivos: store.clientes.filter((cliente) => cliente.hubId === existente.id && cliente.estado === "activo").length, servicios: serviciosPublicos(store, existente.id) }; const baseSlug = slugHub(nombre.replace(/^Hub\s+/i, "")); let slug = baseSlug; let suffix = 2; while (store.hubs.some((hub) => hub.slug === slug)) { slug = `${baseSlug}-${suffix}`; suffix += 1; } const nuevo: Hub = { id: `hub-${Date.now()}`, nombre, slug, zona, estado: "activo", categoriaId: normalizeHubCategory(input.categoriaId), tipo: normalizeHubCategory(input.categoriaId), descripcionPublica: input.descripcionPublica?.trim() || `Hub de demanda de ${zona} para agrupar clientes y coordinar oferta operativa disponible.`, rama: input.rama?.trim() || "General", equipoOperativo: input.equipoOperativo?.trim() || "Equipo a asignar", moduloOperativo: categoria.moduloOperativoSugerido, parametrosOperativos: { jardinerosYa: parametrosJardinerosYaVacios() }, hubOperativo: [], nivelEstabilidad: 8, activo: true, trabajosRealizados: 0, ultimaActividad: timestamp, createdAt: timestamp, updatedAt: timestamp }; store.hubs = [nuevo, ...store.hubs]; await saveStore(store); return { ...nuevo, clientesActivos: 0, servicios: [] }; }
