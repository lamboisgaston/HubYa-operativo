...
export type Cliente = { id: string; nombre: string; email: string; whatsapp: string; referencia?: string; tarifaCliente: TarifaClienteHub; hubId: string; tipoDestino: TipoDestinoContacto; estado: "activo" | "inactivo" | "pendiente_aprobacion"; createdAt: string; updatedAt: string };
export type ContactoInput = Partial<Pick<Cliente, "nombre" | "email" | "whatsapp" | "referencia" | "tarifaCliente" | "hubId" | "tipoDestino">>;
...
function normalizeStore(store: Partial<Store>): Store {
  ...
  const clientes = (store.clientes || seedClientes).map((cliente) => ({ ...cliente, tipoDestino: cliente.tipoDestino || "cliente", tarifaCliente: normalizarTarifaCliente(cliente.tarifaCliente), hubId: idCanonico.get(cliente.hubId) || hubPorClave.get(normalizarClaveHub(cliente.hubId || "")) || cliente.hubId || "" }));
