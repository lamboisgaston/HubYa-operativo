...
export function ContactosMesaTrabajo({ hubs, contactos, hubFijoId = "" }: Props) {
  ...
  const altaDesdeHashAplicada = useRef(false);
  ...
  function agregarCliente() {
    ...
    setMensaje(hubFijoId ? "Completá la fila nueva. El Hub ya quedó asignado automáticamente." : "Completá la fila nueva, seleccioná el Hub asociado y tocá Guardar cambios para agregar el cliente.");
  }

  useEffect(() => {
    if (altaDesdeHashAplicada.current || window.location.hash !== "#agregar") return;
    altaDesdeHashAplicada.current = true;
    agregarCliente();
  }, []);
  ...
  async function guardar(ids?: string[]) {
    ...
    if (sinNombre) { setMensaje("No se puede guardar cliente sin nombre."); return; }
    ...
    const guardados = await Promise.all(pendientes.map((fila) => fetch(fila.nuevo ? "/api/contactos" : `/api/contactos/${fila.id}`, {
      ...
      body: JSON.stringify({ nombre: fila.nombre, email: fila.email, whatsapp: fila.whatsapp, referencia: fila.direccion || fila.referencia || "", direccion: fila.direccion || fila.referencia || "", servicioInteres: fila.servicioInteres || "", observaciones: fila.observaciones || "", tarifaCliente: fila.tarifaCliente, hubId: hubFijoId || fila.hubId, tipoDestino: fila.tipoDestino, estado: fila.estado }),
    }).then(async (res) => { if (!res.ok) { const error = await res.json().catch(() => ({})); throw new Error(error?.error || `No se pudo guardar ${fila.nombre}`); } return res.json() as Promise<Cliente>; }).then((contacto) => ({ temporalId: fila.id, contacto, eraNuevo: Boolean(fila.nuevo) }))));
    ...
    if (nuevos.length === 1) {
      ...
      setMensaje(hub ? `Cliente agregado al Hub ${hub.replace(/^Hub\s+/i, "")}.` : "Cliente agregado al Hub.");
    } else if (nuevos.length > 1) {
      setMensaje(`${nuevos.length} clientes agregados correctamente. Los contadores de Hubs se actualizaron.`);
    } else {
  ...
  return (
    <section id="agregar" className="rounded-[2rem] border border-[#d8dfd1] bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.2em] text-[#7a8a6d]">{hubFijoId ? "Integrantes del Hub" : "Base general"}</p>
          <h2 className="mt-1 text-2xl font-black">{hubFijoId ? "Clientes / vecinos del Hub" : "Clientes en la pantalla principal"}</h2>
          <p className="mt-1 text-sm font-bold text-[#66745c]">Buscá, filtrá por Hub o estado, y editá sin salir del panel. Mostrando {filasFiltradas.length} de {filas.length} clientes.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={agregarCliente} disabled={isPending} className="rounded-xl bg-[#2f6d32] px-4 py-3 text-sm font-black text-white shadow-sm disabled:opacity-60">Agregar cliente</button>
      ...
      <div className="mt-5 grid gap-3 rounded-2xl border border-[#d8dfd1] bg-[#f8faf5] p-4 lg:grid-cols-[1.4fr_1fr_1fr]">
        ...
        <label className="grid gap-1 text-xs font-black uppercase tracking-wide text-[#66745c]">
          ...
          <select value={hubFiltro} onChange={(e) => setHubFiltro(e.target.value)} disabled={Boolean(hubFijoId)} className="rounded-xl border border-[#cfd8c6] bg-white px-4 py-3 text-sm font-bold normal-case">
            ...
            <option value="sin_hub">Sin Hub asociado</option>
        ...
        {filasFiltradas.length === 0 ? <p className="rounded-2xl border border-[#d8dfd1] bg-[#f8faf5] p-6 text-center font-bold text-[#66745c]">No hay contactos que coincidan con la búsqueda.</p> : filasFiltradas.map((fila) => (
          <article key={fila.id} className={`rounded-2xl border border-[#d8dfd1] p-4 ${fila.sucio ? "bg-[#fff8df]" : "bg-white"}`}>
            <div className="flex items-start justify-between gap-3">
              <div><h3 className="font-black">{fila.nombre || "Cliente sin nombre"}</h3><p className="text-sm font-bold text-[#66745c]">{hubPorId.get(fila.hubId) || "Sin Hub asociado"}</p></div>
      ...
      <div className="mt-5 hidden overflow-x-auto rounded-2xl border border-[#d8dfd1] lg:block">
        <table className="w-full min-w-[1280px] border-collapse text-sm">
          <thead className="bg-[#f1f4ec] text-left text-[10px] uppercase tracking-wide text-[#66745c]"><tr>{["Marcar", "Nombre", "Email", "WhatsApp / teléfono", "Dirección / referencia", "Servicio de interés", "Observaciones", "Hub asociado", "Tipo de destino", "Estado", "Acciones"].map((h) => <th key={h} className="border border-[#d8dfd1] p-2">{h}</th>)}</tr></thead>
          <tbody>{filasFiltradas.length === 0 ? <tr><td colSpan={11} className="border p-6 text-center font-bold text-[#66745c]">No hay contactos que coincidan con la búsqueda.</td></tr> : filasFiltradas.map((fila) => <tr key={fila.id} className={fila.sucio ? "bg-[#fff8df]" : "bg-white"}>
            ...
            <td className="border p-1"><select value={fila.hubId} onChange={(e) => actualizar(fila.id, { hubId: e.target.value })} disabled={Boolean(hubFijoId)} className="min-w-48 rounded border bg-white px-2 py-1"><option value="">Sin Hub asociado</option>{hubs.map((hub) => <option key={hub.id} value={hub.id}>{hub.nombre}</option>)}</select></td>
