...
export function MensajesYRespuestasPanel({ hubs, contactos, mensajes, respuestas, parameterResponses, hubActual }: { hubs: Hub[]; contactos: Cliente[]; mensajes: MensajeOperativo[]; respuestas: RespuestaOperativa[]; parameterResponses?: ParameterResponse[]; hubActual?: Hub }) {
  ...
  const resumenParametros = Array.from(respuestasParametros.reduce((mapa, r) => {
    const actual = mapa.get(r.parameterKey) || { label: r.parameterLabel, confirmar_valor: 0, sugerir_otro_valor: 0, sugerir_subir: 0, sugerir_bajar: 0, necesito_aclaracion: 0, sugeridos: new Map<string, number>(), comentarios: 0 };
    const tipo = r.responseType || r.response;
    if (tipo === "confirmar_valor") actual.confirmar_valor += 1;
    if (tipo === "sugerir_otro_valor") actual.sugerir_otro_valor += 1;
    if (tipo === "sugerir_subir") actual.sugerir_subir += 1;
    if (tipo === "sugerir_bajar") actual.sugerir_bajar += 1;
    if (tipo === "necesito_aclaracion") actual.necesito_aclaracion += 1;
    if (r.suggestedValue !== undefined && r.suggestedValue !== "") actual.sugeridos.set(String(r.suggestedValue), (actual.sugeridos.get(String(r.suggestedValue)) || 0) + 1);
    if (r.comment) actual.comentarios += 1;
    mapa.set(r.parameterKey, actual);
    return mapa;
  }, new Map<string, { label: string; confirmar_valor: number; sugerir_otro_valor: number; sugerir_subir: number; sugerir_bajar: number; necesito_aclaracion: number; sugeridos: Map<string, number>; comentarios: number }>()).entries());
