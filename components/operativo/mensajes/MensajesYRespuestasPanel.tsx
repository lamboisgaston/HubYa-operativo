...
const tiposComunicacion = [
  { id: "reporte", label: "Reporte", asunto: "Reporte operativo del Hub", titulo: "Reporte operativo", mensaje: "Hola {nombre}, te compartimos el reporte operativo de {hub}." },
  { id: "informativo", label: "Aviso informativo", asunto: "Información importante del Hub", titulo: "Aviso informativo", mensaje: "Hola {nombre}, te compartimos información importante sobre {hub}." },
  { id: "suspension", label: "Aviso de suspensión", asunto: "Aviso de suspensión", titulo: "Aviso de suspensión", mensaje: "Hola {nombre}, te avisamos una suspensión operativa relacionada con {hub}." },
  { id: "encuesta", label: "Encuesta", asunto: "Encuesta del Hub", titulo: "Queremos conocer tu respuesta", mensaje: "Hola {nombre}, necesitamos tu respuesta sobre {hub}." },
  { id: "parametros", label: "Consulta de parámetros", asunto: "Consulta de parámetros del Hub", titulo: "Consulta de parámetros", mensaje: "Hola {nombre}, queremos consultar parámetros operativos de {hub}." },
];
const tipoMensaje = (m: MensajeOperativo) => {
  const asunto = (m.asunto || "").toLowerCase();
  if (asunto.includes("reporte")) return "Reporte";
  if (asunto.includes("suspensión") || asunto.includes("suspension")) return "Aviso de suspensión";
  if (asunto.includes("parámetro") || asunto.includes("parametro")) return "Consulta de parámetros";
  return m.opcionesRespuesta?.length ? "Encuesta" : "Aviso informativo";
};
...
export function MensajesYRespuestasPanel({ hubs, contactos, mensajes, respuestas, parameterResponses, hubActual, modoInicial = "todo" }: { hubs: Hub[]; contactos: Cliente[]; mensajes: MensajeOperativo[]; respuestas: RespuestaOperativa[]; parameterResponses?: ParameterResponse[]; hubActual?: Hub; modoInicial?: "todo" | "crear" | "respuestas" }) {
  ...
  const [mostrarNuevo, setMostrarNuevo] = useState(modoInicial === "crear");
  const [alcanceTodos, setAlcanceTodos] = useState(false);
  const [hubIds, setHubIds] = useState<string[]>(hubActual ? [hubActual.id] : []);
  const [clienteIds, setClienteIds] = useState<string[]>([]);
  const [tipoComunicacion, setTipoComunicacion] = useState("informativo");
  ...
  const resumenParametros = Array.from(respuestasParametros.reduce((mapa, r) => {
    const actual = mapa.get(r.parameterKey) || { label: r.parameterLabel, confirmar_valor: 0, sugerir_otro_valor: 0, sugerir_subir: 0, sugerir_bajar: 0, necesito_aclaracion: 0 };
    actual[r.responseType] += 1;
    mapa.set(r.parameterKey, actual);
    return mapa;
  }, new Map<string, { label: string; confirmar_valor: number; sugerir_otro_valor: number; sugerir_subir: number; sugerir_bajar: number; necesito_aclaracion: number }>()).entries());
  ...
  function elegirTipo(id: string) {
    const tipo = tiposComunicacion.find((t) => t.id === id);
    if (!tipo) return;
    setTipoComunicacion(id);
    setAsunto(tipo.asunto);
    setTitulo(tipo.titulo);
    setMensaje(tipo.mensaje);
    setIncluyeEncuesta(id === "encuesta" || id === "parametros");
    setPreguntaEncuesta(id === "parametros" ? "¿Confirmás estos parámetros o sugerís cambios?" : "¿Cómo querés avanzar?");
  }
  ...
  return <section className="grid gap-5">
    <header className="rounded-[2rem] border border-[#d8dfd1] bg-white p-6 shadow-sm">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-[#7a8a6d]">Operativo</p><h1 className="mt-1 text-3xl font-black">Comunicaciones</h1>
      <p className="mt-2 max-w-3xl text-sm font-semibold text-[#66745c]">Una ficha de usuario, muchos usos: elegí qué enviar, Hub, destinatarios, mensaje y después mirá las respuestas en el centro general o dentro del Hub.</p>
      {estadoUi && <p className="mt-4 rounded-2xl border border-[#cfd8c6] bg-[#fffdf2] p-3 text-sm font-black">{estadoUi}</p>}
    </header>

    <section className="rounded-[2rem] border border-[#d8dfd1] bg-white p-6 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-2xl font-black">Crear comunicación</h2><p className="text-sm font-semibold text-[#66745c]">Primero elegí qué querés enviar. Después HUBYA guía Hub, destinatarios, mensaje, confirmación, envío y respuestas.</p></div><button onClick={() => setMostrarNuevo((v) => !v)} className="rounded-2xl bg-[#1f2a1d] px-6 py-4 text-sm font-black text-white">Crear nueva comunicación</button></div>
      {mostrarNuevo && <form onSubmit={enviarMensaje} className="mt-6 grid gap-5">
        <div className="rounded-3xl bg-[#f8faf5] p-5"><h3 className="text-lg font-black">¿Qué querés enviar?</h3><div className="mt-3 grid gap-2 md:grid-cols-5">{tiposComunicacion.map((tipo) => <button key={tipo.id} type="button" onClick={() => elegirTipo(tipo.id)} className={`rounded-2xl px-4 py-3 text-sm font-black ${tipoComunicacion === tipo.id ? "bg-[#1f2a1d] text-white" : "bg-white text-[#1f2a1d]"}`}>{tipo.label}</button>)}</div></div>
