...
const campos: Array<{ clave: keyof Omit<ParametrosJardinerosYaHub, "mostrarEnWebPublica">; etiqueta: string; ayuda?: string }> = [
  ...
  { clave: "comisionResponsableCuadrillaPorcentaje", etiqueta: "Comisión del responsable de cuadrilla (%)", ayuda: "Porcentaje destinado al responsable que coordina o maneja la cuadrilla." },
  { clave: "traslado", etiqueta: "Valor de traslado" },
  { clave: "aceite", etiqueta: "Costo de aceite" },
  { clave: "nafta", etiqueta: "Costo de nafta" },
  { clave: "valorHoraCortadoraCesped", etiqueta: "Valor hora máquina cortadora de césped" },
...
export function HubParametrosOperativosEditor({ hub }: { hub: HubPublico }) {
  ...
  async function guardar() {
    ...
    const respuesta = await fetch(`/api/hubs/${hub.slug}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ moduloOperativo, parametrosOperativos: { jardinerosYa: parametros } }) });
    setEstado(respuesta.ok ? "Parámetros operativos del Hub guardados correctamente." : "No se pudieron guardar los parámetros operativos.");
  }

  return <section className="rounded-[2rem] border border-[#bfd3b8] bg-[#fbfdf8] p-6 shadow-sm">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#5d7032]">Parámetros operativos del Hub</p><h2 className="mt-1 text-xl font-black">{moduloOperativo === "jardinerosya" ? "Parámetros JardinerosYa del Hub" : "Módulo operativo asignado"}</h2><p className="mt-2 max-w-3xl text-sm font-semibold text-[#66745c]">El Hub agrupa la demanda. El módulo ordena el proceso. Cada Hub puede tener sus propios parámetros de trabajo.</p></div><button onClick={guardar} className="rounded-2xl bg-[#1f2a1d] px-4 py-3 text-sm font-black text-white">Guardar parámetros</button></div>
    <label className="mt-5 grid gap-1 text-xs font-black uppercase text-[#66745c]">Módulo operativo<select value={moduloOperativo} onChange={(e) => setModuloOperativo(e.target.value as ModuloOperativoHub)} className="h-11 rounded-xl border border-[#cfd8c6] bg-white px-3 text-sm font-semibold normal-case outline-none">{modulos.map((modulo) => <option key={modulo.valor} value={modulo.valor}>{modulo.etiqueta}</option>)}</select></label>
