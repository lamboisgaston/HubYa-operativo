...
export default function Home() {
  ...
  const [seccionActiva, setSeccionActiva] = useState<"reporte" | "informacion" | "importar" | "consultas" | "equipos" | "nuevoHub" | "vinculos">("reporte");
  ...
  useEffect(() => {
    ...
    fetch("/api/hubs", { cache: "no-store" }).then((respuesta) => respuesta.ok ? respuesta.json() : []).then((hubs: HubPublico[]) => {
      ...
      setJornada((actual) => ({ ...actual, datosPorHub: crearDatosParaHubs(actual.datosPorHub, nombres) }));
    }).catch(() => undefined);
    setIsMounted(true);
  }, []);
